...
def _validate_key(key: str) -> None:
    if not isinstance(key, str) or not key:
        raise ValueError(f"memo key must be a non-empty string, got {key!r}")
...
def _validate_base_url(url: str) -> str:
    if not isinstance(url, str) or not url:
        raise ValueError(f"base url must be a non-empty string, got {url!r}")
    return url.rstrip("/")


def _build_url(base_url: str, key: str) -> str:
    return f"{base_url}/memo/api/{key}"
...
def _validate_storage(storage: str) -> None:
    if storage not in VALID_STORAGE:
        raise ValueError(
            f"storage must be one of {sorted(VALID_STORAGE)!r}, got {storage!r}"
        )
...
def _read_settings(path: Optional[Path] = None) -> dict:
    ...
    try:
        ...
        raise ValueError(
            f"{settings_path} contains malformed JSON at line {exc.lineno}, "
    ...
    if (
        ...
        raise ValueError(
            ...
            f"and a tokens object, got {settings!r}"
        )

    for memo_key, token in settings["tokens"].items():
        if not isinstance(memo_key, str) or not isinstance(token, str):
            raise ValueError(
                f"{settings_path} tokens must map strings to strings, "
                f"got invalid entry {memo_key!r}: {token!r}"
            )
    ...
    if not isinstance(settings["keyring_tokens"], list) or not all(
        ...
        raise ValueError(
            f"{settings_path} keyring_tokens must be a list of strings, "
            f"got {settings['keyring_tokens']!r}"
        )
...
def _get_keyring_token(key: str) -> Optional[str]:
    ...
    try:
        ...
        raise KeyringUnavailableError(
            f"could not read token for memo key {key!r} from keyring: {exc}"
        ) from exc
...
def _set_keyring_token(key: str, token: str) -> None:
    ...
    try:
        ...
        raise KeyringUnavailableError(
            f"could not save token for memo key {key!r} to keyring: {exc}"
        ) from exc
...
def _delete_keyring_token(key: str) -> None:
    ...
    try:
        ...
        raise KeyringUnavailableError(
            f"could not delete token for memo key {key!r} from keyring: {exc}"
        ) from exc
...
def _read_token_from_storage(key: str, storage: str) -> Optional[str]:
    token: Optional[str] = None
    if storage in {"file", "both"}:
        token = _get_file_token(key)
    if token is None and storage in {"keyring", "both"}:
        token = _get_keyring_token(key)
    return token
...
def _resolve_token(key: str, token: Optional[str]) -> str:
    ...
    raise TokenNotFoundError(
        f"no token provided for memo key {key!r} and none found in "
        f"environment variable {env_var!r}, settings file "
        f"{_token_settings_path()!s}, or {keyring_note}"
    )
...
def _validate_value(value: dict) -> None:
    ...
    try:
        ...
        raise ValueError(f"push value must be JSON serializable: {exc}") from exc

    if len(json_bytes) > MAX_VALUE_BYTES:
        raise ValueError(
            f"push value JSON payload must be at most {MAX_VALUE_BYTES} bytes, "
            f"got {len(json_bytes)} bytes"
        )


def _response_body(response: requests.Response, *, limit: int = 1024) -> str:
    body = response.text
    if not body:
        return "<empty>"
    if len(body) > limit:
        return f"{body[:limit]}... ({len(body)} bytes total)"
    return body
...
def _parse_response(response: requests.Response) -> Optional[object]:
    ...
    if not response.ok:
        ...
        raise RuntimeError(
            f"{method} {url} returned HTTP {response.status_code}{reason}; "
            f"body: {_response_body(response)!r}"
        )
...
def _request_json(method, url: str, **kwargs) -> Optional[object]:
    ...
    try:
        ...
        raise RuntimeError(f"{method_name} {url} request failed: {exc}") from exc
...
def get_value(
    ...
    *,
    ...
    if not isinstance(response, dict) or "value" not in response:
        raise ValueError(
            f"memo key {key!r} response has no value field; received JSON: {response!r}"
        )
...
def copy_token(
    ...
    *,
    ...
    if src_storage is not None:
        _validate_storage(src_storage)
    if dst_storage is not None:
        _validate_storage(dst_storage)

    if src_storage is not None:
        token = _read_token_from_storage(src_key, src_storage)
        if not token:
            raise TokenNotFoundError(
                f"no token found for memo key {src_key!r} in {src_storage!r} storage"
            )
        resolved_src_storage: Literal["file", "keyring", "both"] = src_storage
    else:
        token = _resolve_token(src_key, None)
        resolved_src_storage = cast(
            Literal["file", "keyring", "both"],
            get_token_storage(src_key),
        )

    effective_dst_storage = (
        dst_storage if dst_storage is not None else resolved_src_storage
    )
    save_token(dst_key, token, storage=effective_dst_storage)
